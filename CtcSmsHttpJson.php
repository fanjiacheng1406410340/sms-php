...
function checkKeyword($content) {
	// 检测敏感词数据包json格式：{"account":"8528","password":"e717ebfd5271ea4a98bd38653c01113d","content":"低价处理二手车"}
	$data = array ('account' => ACCOUNT, 'password' => PASSWORD, 'content' => $content );
	return http_post_json ( __FUNCTION__, URL . "/KeywordCheck", json_encode ( $data ) );
}
/**
 * 检测黑名单
 * 
 */
function checkBlackList($phones) {
	// 检测黑名单数据包json格式：{"account":"8528","password":"e717ebfd5271ea4a98bd38653c01113d","phones":"15711666132"}
	$data = array ('account' => ACCOUNT, 'password' => PASSWORD, 'phones' => $phones );
	return http_post_json ( __FUNCTION__, URL . "/BlackListCheck", json_encode ( $data ) );
}
/**
